...
//******************************************************************************************
#define VOLTAGE_CALC  1.08
#define CURRENT_A 132.7
#define CURRENT_B 1327/5
#define POWER_A   0.1502
#define POWER_B   0.751
...
float ADE7953::getVrms(){  
  ...
  float decimal = voltageCalcMethod->getResult(value);
	//float decimal = decimalize(value, 19090/VOLTAGE_CALC, 0); //convert to float with calibration factors specified
return decimal;
  }  
...
float ADE7953::getIrmsA(){  
  ...
  Serial.print("getIrmsA(hex): ");
  Serial.println(value);
  float decimal = currentCalcMethod->getResult(value);
	//float decimal = decimalize(value, CURRENT_A, 0); //convert to float with calibration factors specified
return decimal;
  }
float ADE7953::getIrmsB(){  
  unsigned long value=0;  
  value=spiAlgorithm32_read((functionBitVal(IRMSB_32,1)),(functionBitVal(IRMSB_32,0))); 
  Serial.print("getIrmsB(hex): ");
  Serial.println(value);
  float decimal = currentCalcMethod->getResult(value);
  //float decimal = decimalize(value, CURRENT_B, 0); //convert to float with calibration factors specified
return decimal;
  }  
...
float ADE7953::getInstApparentPowerA(){  
  ...
  Serial.print("getInstApparentPowerA(hex): ");
  Serial.println(value);
	float decimal = decimalize(value, POWER_A*VOLTAGE_CALC, 0); //convert to float with calibration factors specified
return abs(decimal);
  }
float ADE7953::getInstApparentPowerB(){  
  long value=0;  
  value=spiAlgorithm32_read((functionBitVal(BVA_32,1)),(functionBitVal(BVA_32,0))); 
  Serial.print("getInstApparentPowerB(hex): ");
  Serial.println(value);
  float decimal = decimalize(value, POWER_B*VOLTAGE_CALC, 0); //convert to float with calibration factors specified
return abs(decimal);
  }  
...
float ADE7953::getInstActivePowerA(){  
  ...
  Serial.print("getInstActivePowerA(hex): ");
  Serial.println(value);
	float decimal = decimalize(value, POWER_A*VOLTAGE_CALC, 0); //convert to float with calibration factors specified
return abs(decimal);
  }
float ADE7953::getInstActivePowerB(){  
  ...
  Serial.print("getInstActivePowerB(hex): ");
  Serial.println(value);
  float decimal = decimalize(value, POWER_B*VOLTAGE_CALC, 0); //convert to float with calibration factors specified
return abs(decimal);
  }  
...
float ADE7953::getInstReactivePowerA(){  
	...
	float decimal = decimalize(value, 1.502/VOLTAGE_CALC, 0); //convert to float with calibration factors specified
return decimal;
  }
...
//****************Object Definition*****************
ADE7953::ADE7953(int SS, uint32_t SPI_freq)
{
  _SS=SS;
  _SPI_freq=SPI_freq;
  }
...
void ADE7953::initialize(){
    ...
    currentCalcMethod=new CalcMehtod(CURRENT_GAIN, CURRENT_BURDEN, CURRENT_RATIO);
    voltageCalcMethod=new CalcMehtod(VOLTAGE_GAIN, VOLTAGE_BURDEN, VOLTAGE_RATIO);
  ...
  delay(100);
  byte gainval = getGainValue(CURRENT_GAIN);
  spiAlgorithm8_write((functionBitVal(PGA_IA_8,1)),(functionBitVal(PGA_IA_8,0)),gainval); //
  delay(100);
  spiAlgorithm8_write((functionBitVal(PGA_IB_8,1)),(functionBitVal(PGA_IB_8,0)),gainval); //
  delay(100);

}
//**************************************************
byte ADE7953::getGainValue(int gain){
  byte ret = 0;
  switch(gain){
    case 2: ret = 0b00000001; break;
    case 4: ret = 0b00000010; break;
    case 8: ret = 0b00000011; break;
    case 16: ret = 0b00000100; break;
    case 22: ret = 0b00000101; break;
  }
  return ret;
}
